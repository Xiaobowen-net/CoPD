...
class SMD_OriSegLoader(object):
    def __init__(self, index, data_path, win_size, step, mode="train"):
        ...
        # data = np.load(data_path + "/machine-" + args.fname1 + "_train.pkl", allow_pickle=True)
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load(data_path + "/SMD_Ori_"+str(index)+"_test.npy")
        # test_data = np.load(data_path + "/machine-" + config.fname1 + "_test.pkl", allow_pickle=True)
        self.test = self.scaler.transform(test_data)

        self.train = data
        self.val = self.test
        self.test_labels = np.load(data_path + "/SMD_Ori_"+str(index)+"_test_label.npy")
        # self.test_labels = np.load(data_path + "/machine-" + config.fname1 + "_test_label.pkl", allow_pickle=True)
        if self.mode == "val":
            print("train:", self.train.shape)
            print("test:", self.test.shape)
...
class multidatasetSegLoader(Dataset):
    def __init__(self, data_path, win_size, step, mode="train"):
        self.mode = mode
        self.step = step
        self.win_size = win_size
...
class unidatasetSegLoader(Dataset):
    def __init__(self, data_path, win_size, step, mode="train"):
        self.mode = mode
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = pd.read_csv(data_path + '/0_train.csv')
        data = data.values[:, :1]
        data = np.nan_to_num(data)
        # self.scaler.fit(data)
        # data = self.scaler.transform(data)
        test_data = pd.read_csv(data_path + '/0_test.csv')
        ...
        self.train = data
        self.val = self.test
        self.test_labels = pd.read_csv(data_path + '/0_test_label.csv').values[:, 1:]
...
def get_loader_segment(index, data_path, batch_size, win_size=100, step=100, mode='train', dataset='KDD'):
    if (dataset == 'SMD'):
        ...
        dataset = multidatasetSegLoader(data_path, win_size, 1, mode)
    elif (dataset == 'unidataset'):
        dataset = unidatasetSegLoader(data_path, win_size, 1, mode)
