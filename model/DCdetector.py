...
class DCdetector(nn.Module):
    def __init__(self, win_size, enc_in, c_out, n_heads=1, d_model=256, e_layers=3, patch_size=[3, 5, 7], channel=55,
        ...
        self.decompsition_multi = series_decomp_multi(moving_avg)
        ...
        self.dropout_series1 = nn.Dropout(0.05)
        self.dropout_series2 = nn.Dropout(0.05)
        ...
        self.Decomp_DFT = DFT_series_decomp()
        # self.trendMLP = nn.Linear(win_size, win_size)
        # self.trendMLP.weight = nn.Parameter((1 / win_size) * torch.ones([win_size, win_size]))
        # Encoder
        self.trendMLP = nn.Sequential(
            nn.Linear(win_size, d_model),
            nn.ReLU(),
            nn.Linear(d_model, d_model),
            nn.ReLU(),
    ...
    def forward(self, x_enc):
        ...
        x, trend = self.decompsition_multi(x_enc)
        # x, trend = self.Decomp_DFT(x_enc)
        ...
        trend = revin_layer(trend, 'norm')
        trend_rec = self.trendMLP(trend.permute(0, 2, 1)).permute(0, 2, 1)
        trend_rec = revin_layer(trend_rec, 'denorm')
        ...
        # revin_layer1 = RevIN(num_features=M)
        # x0 = revin_layer1(rec, 'norm')
        x0 = rec
        ...
        for patch_index, patchsize in enumerate(self.patch_size):
            ...
            # x_patch_size = rearrange(x_patch_size, 'b (n p) m -> (b n) p m', p=patchsize)

            V1, series1 = self.encoder_ALi[0](x_patch_num, patch_index, tau=None, delta=None)
            # series_patch_mean.append(series1)
            x_patch_size = rearrange(V1, '(b p) n m -> (b n) p m', p=patchsize)
