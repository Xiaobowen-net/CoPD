...
class DCdetector(nn.Module):
    def __init__(self, win_size, enc_in, c_out, n_heads=1, d_model=256, e_layers=3, patch_size=[3, 5, 7], channel=55,
        ...
        self.dropout_series = nn.Dropout(0.05)
        ...
        self.pro_x_patch_size = nn.ModuleList()
        for i, patchsize in enumerate(self.patch_size):
            self.embedding_patch_size.append(DataEmbedding(patchsize, d_model, dropout))
            self.embedding_patch_num.append(DataEmbedding(win_size // patchsize, d_model, dropout))
            self.pro_x_patch_size.append(nn.Linear(d_model, patchsize))
    ...
    def forward(self, x_enc):
        ...
        for patch_index, patchsize in enumerate(self.patch_size):
            ...
            x_patch_size, x_patch_num = x0, x0
            ...
            x_patch_size, x_patch_size_series = self.encoder_ALi[0](x_patch_size, patch_index, tau=None, delta=None)
            x_patch_num = self.pro_x_patch_size[patch_index](x_patch_size).permute(0,2,1)
            x_patch_num = self.embedding_patch_num[patch_index](x_patch_num)
            x_patch_num, x_patch_num_series = self.encoder_ALi[1](x_patch_num, patch_index, tau=None, delta=None)

            # for index, ser in enumerate(series2):
            #     ser = self.dropout_series(ser)
            series = x_patch_size_series + x_patch_num_series
